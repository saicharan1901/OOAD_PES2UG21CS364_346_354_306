...
# Route for rendering the criminal registration form
@app.route('/criminal-registration', methods=['GET'])
def criminal_registration_form():
    return render_template('criminal_registration.html')

# Route for processing the criminal registration form submission
...
def register_criminal():
    ...
    distinct_features = request.form['distinct_features']

    # Insert into database
    sql = "INSERT INTO criminals (name, crime_type, age, gender, ethnicity, distinct_features) VALUES (%s, %s, %s, %s, %s, %s)"
    val = (name, crime_type, age, gender, ethnicity, distinct_features)
    cursor.execute(sql, val)
    db.commit()

    return jsonify({'message': 'Criminal registered successfully'})

# Route for rendering a success page after criminal registration
